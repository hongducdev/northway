 ...
 * @package Case-Themes
 */
add_action('after_setup_theme', 'northway_setup');
function northway_setup(){

    //Set the content width in pixels, based on the theme's design and stylesheet.
    $GLOBALS['content_width'] = apply_filters( 'northway_content_width', 1200 );

    // Make theme available for translation.
    load_theme_textdomain( 'northway', get_template_directory() . '/languages' );
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary Desktop', 'northway' ),
        'primary-mobile' => esc_html__( 'Primary Mobile', 'northway' ),
    ) );
    ...
    // Enable support for Post Thumbnails on posts and pages.
    add_theme_support('post-thumbnails');
    add_image_size( 'northway-thumb-small', 80, 70, true );
    add_image_size( 'northway-thumb-xs', 120, 104, true );
    add_image_size( 'northway-large', 952, 333, true );
    add_image_size( 'northway-thumb-related', 767, 653, true );
    add_image_size( 'northway-portfolio', 600, 600, true );
 ...
 */
add_action( 'widgets_init', 'northway_widgets_position' );
function northway_widgets_position() {
    register_sidebar( array(
        'name'          => esc_html__( 'Blog Sidebar', 'northway' ),
        'id'            => 'sidebar-blog',
    ...
    if (class_exists('ReduxFramework')) {
        register_sidebar( array(
            'name'          => esc_html__( 'Page Sidebar', 'northway' ),
            'id'            => 'sidebar-page',
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
    ...
    if ( class_exists( 'Woocommerce' ) ) {
        register_sidebar( array(
            'name'          => esc_html__( 'Shop Sidebar', 'northway' ),
            'id'            => 'sidebar-shop',
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title"><span>',
 ...
 */
add_action( 'wp_enqueue_scripts', 'northway_scripts' );
function northway_scripts() {  
    $northway_version = wp_get_theme( get_template() );
    /* Popup Libs */
    ...
    /* Parallax Scroll */
    wp_enqueue_script( 'pxl-parallax-background', get_template_directory_uri() . '/assets/js/libs/parallax-background.js', array( 'jquery' ), $northway_version->get( 'Version' ), true );
    wp_enqueue_script( 'pxl-parallax-scroll', get_template_directory_uri() . '/assets/js/libs/parallax-scroll.js', array( 'jquery' ), $northway_version->get( 'Version' ), true );
    ...
    /* Woocommerce */
    wp_enqueue_script( 'pxl-woocommerce', get_template_directory_uri() . '/woocommerce/js/woocommerce.js', array( 'jquery' ), $northway_version->get( 'Version' ), true );

    /* Icon */
    wp_enqueue_style('bootstrap-icons', get_template_directory_uri() . '/assets/fonts/bootstrap-icons/css/bootstrap-icons.css');
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/flaticon/css/flaticon.css' , array(), $northway_version->get( 'Version' ));
    /* Cookie */
    ...
    wp_enqueue_style( 'pxl-grid', get_template_directory_uri() . '/assets/css/grid.css', array(), $northway_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-style', get_template_directory_uri() . '/assets/css/style.css', array(), $northway_version->get( 'Version' ) );
    wp_add_inline_style( 'pxl-style', northway_inline_styles() );
    wp_enqueue_style( 'pxl-base', get_template_directory_uri() . '/style.css', array(), $northway_version->get( 'Version' ) );
    wp_enqueue_style( 'pxl-google-fonts', northway_fonts_url(), array(), null );
    wp_enqueue_script( 'pxl-main', get_template_directory_uri() . '/assets/js/theme.js', array( 'jquery' ), $northway_version->get( 'Version' ), true );
    wp_localize_script( 'pxl-main', 'main_data', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
        wp_enqueue_script( 'comment-reply' );
    }
    do_action( 'northway_scripts');
}

/**
 * Enqueue Styles Scripts : Back-End
 */
add_action('admin_enqueue_scripts', 'northway_admin_style');
function northway_admin_style() {
    $theme = wp_get_theme( get_template() );
    wp_enqueue_style( 'northway-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), $theme->get( 'Version' ) );
    wp_enqueue_style('bootstrap-icons', get_template_directory_uri() . '/assets/fonts/bootstrap-icons/css/bootstrap-icons.css');
    wp_enqueue_style('flaticon', get_template_directory_uri() . '/assets/fonts/flaticon/css/flaticon.css');
}

add_action( 'elementor/editor/before_enqueue_scripts', function() {
    wp_enqueue_style( 'northway-admin-style', get_template_directory_uri() . '/assets/css/admin.css');
...
/* Favicon */
add_action('wp_head', 'northway_site_favicon');
function northway_site_favicon(){
    $favicon = northway()->get_theme_opt( 'favicon' );
 ...
 */
add_action( 'wp_head', 'northway_pingback_header' );
function northway_pingback_header(){
...
/* Hidden Panel */
add_action( 'pxl_anchor_target', 'northway_hook_anchor_templates_hidden_panel');
function northway_hook_anchor_templates_hidden_panel(){

    $hidden_templates = northway_get_templates_slug('hidden-panel');
...
if(!function_exists('northway_hook_anchor_hidden_panel')){
    function northway_hook_anchor_hidden_panel($args){
        ...
        <div class="pxl-hidden-panel-popup pxl-hidden-template-<?php echo esc_attr($args['post_id'])?> pxl-pos-<?php echo esc_attr($hidden_panel_position); ?>">
            ...
            <div class="pxl-popup--content" style="height:<?php echo esc_attr($hidden_panel_height).'px'; ?>; background-color:<?php echo esc_attr($hidden_panel_boxcolor); ?>;">
...
/* Elementor Popup */
add_action( 'pxl_anchor_target', 'northway_hook_anchor_templates_popup');
function northway_hook_anchor_templates_popup(){

    $popup_templates = northway_get_templates_slug('popup');
...
if(!function_exists('northway_hook_anchor_popup')){
    function northway_hook_anchor_popup($args){ ?>
...
/* Page Popup */
add_action( 'pxl_anchor_target', 'northway_hook_anchor_templates_page_popup');
function northway_hook_anchor_templates_page_popup(){

    $page_templates = northway_get_templates_slug('popup');
...
if(!function_exists('northway_hook_anchor_page_popup')){
    function northway_hook_anchor_page_popup($args){ ?>
        <div class="pxl-page-popup pxl-page-popup-template-<?php echo esc_attr($args['post_id'])?>">
            <div class="pxl-close-popup  pxl-cursor--cta">
                <span class="pxl-close"></span>
            </div>
...
/* Cart Sidebar */
if(!function_exists('northway_hook_anchor_cart')){
    function northway_hook_anchor_cart(){ 
        global $woocommerce; ?>
...
add_action('wp_ajax_nopriv_item_added', 'northway_addedtocart_sweet_message');
add_action('wp_ajax_item_added', 'northway_addedtocart_sweet_message');
function northway_addedtocart_sweet_message() {
...
add_action('wp_footer', 'northway_cart_hidden_sidebar');
function northway_cart_hidden_sidebar() {
