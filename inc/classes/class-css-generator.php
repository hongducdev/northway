...
class Northway_CSS_Generator
{
    ...
    {
        $this->opt_name = northway()->get_option_name();
        ...
        add_action('wp_enqueue_scripts', array($this, 'northway_enqueue'), 20);
    }
    ...
    function init()
    {
        ...
        add_action('wp', array($this, 'northway_generate_with_dev_mode'));
        add_action("redux/options/{$this->opt_name}/saved", function () {
            $this->northway_generate_file_options();
        });
    }

    function northway_generate_with_dev_mode()
    {
        if ($this->dev_mode === true) {
            $this->northway_generate_file_options();
            $this->northway_generate_file();
        }
    }

    function northway_generate_file_options()
    {
        ...
        $this->redux->filesystem->execute('put_contents', $_options, array(
            'content' => preg_replace("/(?<=[^\r]|^)\n/", "\r\n", $this->northway_options_output())
        ));
    }

    function northway_generate_file()
    {
        $scss_dir = get_template_directory() . '/assets/scss/';
    ...
    protected function northway_options_output()
    {
        $theme_colors                    = northway_configs('theme_colors');
        $links                           = northway_configs('link');
        $gradients                       = northway_configs('gradient');
        $gradient_color_two        = northway_configs('gradient_two');
        ob_start();

        printf('$northway_theme_colors_key:(%s);', $this->print_scss_opt_colors($theme_colors, 'key'));
        printf('$northway_theme_colors_val:(%s);', $this->print_scss_opt_colors($theme_colors, 'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function northway_enqueue()
    {
        $css = $this->northway_inline_css();
        if (!empty($css)) {
            wp_add_inline_style('pxl-style', $css);
        }
    }
    protected function northway_inline_css()
    {
        ob_start();
