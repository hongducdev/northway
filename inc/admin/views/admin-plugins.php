...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'northway');
...
foreach( $plugins as $plugin ){
	$file_path = $plugin['file_path'];
	if ( !$admin_plugin_class->pxl_plugin_is_active($file_path) ) {
		$status_act = true;
		break;
	}
}

$merlin_setup = get_option( 'merlin_' . northway()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'northway');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'northway');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'northway');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'northway' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'northway' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'northway' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Northway_Admin_Plugins;
