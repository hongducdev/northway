(function ($) {
    ...
    function initTabs() {
        $(document).on("click", ".pxl-tab-nav > ul > li > a", function () {
            ...
            $(this)
                .closest(".pxl-demos")
                .find(".pxl-col:not(." + data_filter + ")")
                .css("display", "none");
            $(this)
                .closest(".pxl-demos")
                .find(".pxl-col." + data_filter)
                .css("display", "flex");
        });
    }
    ...
    function initDemo() {
        $(".pxl-demos").on("click", ".pxl-popup-import", function () {
            ...
            var demo_file_url =
                api_url + "demos/" + theme_slug + "/" + id + ".zip";
            $.ajax({
                url: pxl_ajax_url,
                type: "GET",
                data: {
                ...
                if (jsonresp.stat === 1) {
                    ...
                    $(".pxl-demo-loader").removeClass("active");
                    var $content =
                        "Your server was unable to connect to theme API server";
                    if (jsonresp.stat === 0 && jsonresp.message != null) {
                        $content = jsonresp.message;
                    }
                    $(".pxl-form-upload-demo").css("display", "block");
                    ...
                    $(".pxl-demo-error-confirm")
                        .find(".link-download-demo-manual")
                        .html('<a href="' + demo_file_url + '">Click Here</a>');

                    $(".pxl-form-upload-demo").on(
                        "click",
                        ".btn-upload",
                        function () {
                            var file_data = $(
                                'input[name="demo_filename"]'
                            ).prop("files")[0];
                            var nonce = $(
                                "#pxlart_upload_demo_manual_nonce"
                            ).val();
                            var form_data = new FormData();
                            form_data.append("file", file_data);
                            form_data.append(
                                "action",
                                "pxlart_upload_demo_manual"
                            );
                            ...
                            $.ajax({
                                ...
                                beforeSend: function () {
                                    ...
                                    $(
                                        ".pxl-demo-error-confirm .btn-upload"
                                    ).addClass("loading");
                                },
                            }).done(function (res) {
                                $(
                                ...
                                if (res == "1") {
                                    $(".pxl-demo-error-confirm").removeClass(
                                        "is-active"
                                    );
                                    $(".pxl-demo-content").addClass("active");
                                    initPopUp(id);
                                    ...
                                    $(".pxl-demo-error-confirm")
                                        .find(".pxl-form-upload-demo")
                                        .append(
                                            '<p class="err">File upload incorect or not found</p>'
                                        );
                                } else if (res == "3") {
                                    $(".pxl-demo-error-confirm")
                                        .find(".pxl-form-upload-demo")
                                        .append(
                                            '<p class="err">The setting for upload_max_filesize is smaller than file upload, try change upload max file size in php config from 64MB or greater</p>'
                                        );
                                } else {
                                    $(".pxl-demo-error-confirm")
                                        .find(".pxl-form-upload-demo")
                                        .append(
                                            '<p class="err">File upload is false!</p>'
                                        );
                                }
                            });
                        }
        ...
        $(document).on(
            "click",
            ".pxl-demo-error-confirm .confirm-footer .btn",
            function () {
                $(this)
                    .closest(".pxl-demo-error-confirm")
    ...
    function initPopUp(demo) {
        ...
        $(".pxl-demo-content").on("click", ".pxl-import-btn", function () {
            var options = [];
            $(this)
                .closest(".pxl-demo-content")
                .find(" .pxl-imp-opt :checked")
                .each(function () {
            ...
            var crop_img = "yes";
            var skip_posts = "yes";
            var crop_img_checked = $(this)
                .closest(".pxl-demo-content")
                .find(" .pxl-imp-opt-crop :checked")
                .val();
            var skip_posts_existen = $(this)
                .closest(".pxl-demo-content")
                .find(" .pxl-imp-opt-skip-posts :checked")
                .val();
            ...
            var importer = new pxlartImporter(
                demo,
                options,
                crop_img,
                skip_posts
    ...
    function runImport(options, id, crop_img, skip_posts) {
        ...
        options[count] &&
            ajaxRun(
                "pxlart_" + options[options.length - options.length],
                options,
                id,
                count,
                crop_img,
    ...
    function PxlPluginManager() {
        ...
        return {
            init: function () {
                ...
                $(".pxl-dsb-plugin:not(.pxl-dsb-plugin-active)").addClass(
                    "installing"
                );
                complete = function () {
                    setTimeout(function () {
            ...
            var image = wp
                ...
                .on("select", function (e) {
                    ...
                    $this
                        ...
                        .find(".pxl-show-image")
                        .append(
                            '<img src = "' + uploaded_image.toJSON().url + '">'
                        );
                    $this.hide();
                    $this.parent().find(".pxl-remove-image").show();
                    $this
                        .parents("form")
                        .find('input[name="savewidget"]')
                        .removeAttr("disabled");
                });
        });
